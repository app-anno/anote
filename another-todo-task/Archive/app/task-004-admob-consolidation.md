...
広告プラットフォームをAppLovinからAdMobに完全統一し、ABテストを終了して全ユーザーに広告を表示する（サブスクユーザーのみ非表示）。インタースティシャル広告は「3回に1回」表示に固定。
...
| **ABテスト終了** | 広告のABテストを終了、全ユーザーに表示 |
| **サブスク非表示** | サブスクユーザーのみ広告を非表示 |
| **インタースティシャル3回に1回** | エリア詳細画面遷移3回ごとに表示 |
...
| ファイル | 内容 | 本タスクとの関連 |
|---------|------|-----------------|
| `app/views/common/AdMobInterstitialManager.ts` | AdMobインタースティシャル広告管理 | ABテストロジック削除、頻度固定化 |
| `app/views/common/AppLovinBanner.tsx` | AppLovinバナー広告 | **削除対象** |
| `app/views/common/AppLovinMREC.tsx` | AppLovin MREC広告 | **削除対象** |
| `app/views/common/HybridAdBanner.tsx` | ハイブリッド広告コンポーネント | AdMob用に書き換え、ABテスト削除 |
| `app/helpers/ABTestHelper.ts` | ABテスト管理 | 広告関連のABテストロジック削除 |
...
**変更内容:** AppLovinをAdMobに置き換え、ABテストロジックを削除してシンプル化
...
// - abTestAdEnabled関連
// - AB_TEST_AD_ENABLED_KEY

// 広告以外のABテスト機能があれば残す
// 広告関連のみであれば、ファイル全体を削除も検討
```
...
pod 'GoogleMobileAdsMediationPangle', '6.3.0.6'          # Pangle (iOSのみ)
```
...
|-----------|-----|---------|------|
| **Moloco** | ❌ | ❌ | SDK 12.3.0+必須 |
| **Verve Group** | ❌ | ❌ | SDK 13.0.0+必須 |
| **BIGO Ads** | ❌ | ❌ | SDK 12.4.1+必須 |
| **Chartboost** | ❌ | ❌ | SDK 13.0.0+必須 |
...
| 6 | 非サブスク会員の広告表示 | 非サブスク会員でログイン | 全ての広告が表示される |
| 7 | フォールバック動作 | 機内モードで広告画面を開く | 自社広告（NativeAd）が表示される |
| 8 | 複数ユニットID | 異なる画面で広告確認 | adIndexに応じた異なるユニットIDが使用される |
...
- [ ] `app/views/common/HybridAdBanner.tsx` をAdMob用に書き換え（ABテスト削除）
- [ ] `app/views/common/AdMobInterstitialManager.ts` からABテストロジックを削除、3回に1回固定
- [ ] `app/helpers/ABTestHelper.ts` から広告関連のABテストロジックを削除

### Phase 3: AppLovin関連の削除
...
| `app/views/common/HybridAdBanner.tsx` | AdMob用に書き換え、ABテスト削除 |
| `app/views/common/AdMobInterstitialManager.ts` | ABテストロジック削除、3回固定 |
| `app/helpers/ABTestHelper.ts` | 広告関連ABテスト削除 |
...
| Q13 | メディエーションアダプター | 互換性のあるもののみ追加 |
| Q14 | ABテスト | 終了。全ユーザー表示（サブスクのみ非表示） |
